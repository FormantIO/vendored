...
func (a *azureObjects) CheckAbandonedParts(ctx context.Context, bucket, object string, opts madmin.HealOpts) error {
	return nil
}

func (a *azureObjects) DecomTieredObject(ctx context.Context, bucket, object string, fi minio.FileInfo, opts minio.ObjectOptions) error {
	return nil

}

func (a *azureObjects) MakeBucket(ctx context.Context, bucket string, opts minio.MakeBucketOptions) error {
	return nil
}

func (a *azureObjects) NSScanner(ctx context.Context, channel chan<- minio.DataUsageInfo, wat uint32, mode madmin.HealScanMode) error {
	return nil
}
...
func (a *azureObjects) GetDisks(l int, r int) ([]minio.StorageAPI, error) {
	return nil, nil
}
...
func (a *azureObjects) ListBuckets(ctx context.Context, opts minio.BucketOptions) (buckets []minio.BucketInfo, err error) {
	marker := azblob.Marker{}
...
func (a *azureObjects) GetObjectNInfo(ctx context.Context, bucket, object string, rs *minio.HTTPRangeSpec, h http.Header, opts minio.ObjectOptions) (gr *minio.GetObjectReader, err error) {
...
func (a *azureObjects) CopyObject(ctx context.Context, srcBucket, srcObject, destBucket, destObject string, srcInfo minio.ObjectInfo, srcOpts, dstOpts minio.ObjectOptions) (objInfo minio.ObjectInfo, err error) {
	...
	res, err := destBlob.StartCopyFromURL(ctx, srcBlobURL, azureMeta, azblob.ModifiedAccessConditions{}, azblob.BlobAccessConditions{},
		azblob.AccessTierHot, azblob.BlobTagsMap{})
	if err != nil {
		return objInfo, azureToObjectError(err, srcBucket, srcObject)
	}
	...
	if _, ok := srcInfo.UserDefined["x-amz-storage-class"]; ok {
		_, err = destBlob.SetTier(ctx, s3StorageClassToAzureTier(srcInfo.UserDefined["x-amz-storage-class"]),
			azblob.LeaseAccessConditions{}, azblob.RehydratePriorityHigh)
		if err != nil {
			return objInfo, azureToObjectError(err, srcBucket, srcObject)
		}
	}
...
// func (a *azureObjects) NewMultipartUpload(ctx context.Context, l string, n string, opts minio.ObjectOptions) (*minio.NewMultipartUploadResult, error) {
// 	return nil, nil
// }

// NewMultipartUpload - Use Azure equivalent `BlobURL.Upload`.
func (a *azureObjects) NewMultipartUpload(ctx context.Context, bucket, object string, opts minio.ObjectOptions) (res *minio.NewMultipartUploadResult, err error) {
	...
	_, err = blobURL.Upload(ctx, bytes.NewReader(jsonData), azblob.BlobHTTPHeaders{}, azblob.Metadata{}, azblob.BlobAccessConditions{}, azblob.AccessTierHot, azblob.BlobTagsMap{}, azblob.ClientProvidedKeyOptions{}, azblob.ImmutabilityPolicyOptions{})
	if err != nil {
		return nil, azureToObjectError(err, bucket, metadataObject)
	}

	return &minio.NewMultipartUploadResult{
		UploadID:     uploadID,
		ChecksumAlgo: hash.MinIOMultipartChecksum,
	}, nil
}
...
func (a *azureObjects) CompleteMultipartUpload(ctx context.Context, bucket, object, uploadID string, uploadedParts []minio.CompletePart, opts minio.ObjectOptions) (objInfo minio.ObjectInfo, err error) {
	...
	_, err = objBlob.CommitBlockList(ctx, allBlocks, objProperties, objMetadata, azblob.BlobAccessConditions{}, azblob.AccessTierHot, azblob.BlobTagsMap{}, azblob.ClientProvidedKeyOptions{}, azblob.ImmutabilityPolicyOptions{})
	if err != nil {
		return objInfo, azureToObjectError(err, bucket, object)
	}
	var partNumberMarker int
...
func (a *azureObjects) SetBucketPolicy(ctx context.Context, bucket string, bucketPolicy *policy.BucketPolicy) error {
